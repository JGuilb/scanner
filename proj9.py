...
def name_image(x):
    ...
    if (x<10):
        ...
        name = 'n_'+ str(x)
        real_n = name+'.png'
    return real_nhtml télécharger un lien avec commentaires
...
def getNewName(x,y):
    ...
    if x==1:
        if y==0:
            name = "A_"+ str(x+1) +".png"  
        if y==1:
            name = "B_"+ str(x+1) +".png"  html télécharger un lien avec commentaires
        if y==2:
            name = "C_"+ str(x+1) +".png"
    return name
...
def cut(plaq,puits,acc,i):
    ...
    if (puits==12):
        xsize = largeur//3
        ysize = hauteur//4
        cut_plaq(plaq,image,largeur,hauteur,xsize,ysize,4,3,i)
    if (puits==24):
        xsize = largeur//4
        ysize = hauteur//6
        cut_plaq(plaq,image,largeur,hauteur,xsize,ysize,6,4,i)
